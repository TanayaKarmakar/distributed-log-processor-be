...
class LogGenerator:
    ...
    def _get_log_source_and_user(self):
        user_id = str(uuid.uuid4())
        log_source = random.choices(self.config["SERVICES"], k=1)[0]
        return f"[{log_source}]: User-{user_id}"
    ...
    def generate_log_message(self):
        ...
        log_source_and_user = self._get_log_source_and_user()
